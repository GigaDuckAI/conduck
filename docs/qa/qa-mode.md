...
| `-ConduckUncapCustomGateways` | Lift the combined configured-gateway allowance (`Constants.maxConfiguredGateways`, also exposed as `Constants.maxCustomGateways`) to the badge-palette ceiling (`RemoteAgentBadgePalette.customPalette.count`) for a dev/QA fleet. OpenRouter remains exempt. Lives beside the constant in `Utilities/Constants.swift`, NOT in `QA/DebugFlags.swift`: the Watch target compiles that file but not `QA/`. Read once at launch; Release builds contain no override path. Pre-added unticked to the shared scheme's Run ▸ Arguments; leave it off when checking the free allowance. |
| `-ConduckInitializeCloudKitSchema` | One-shot CloudKit **Development** schema fill before a Production deploy. Core Data creates record types and fields lazily, as records carrying them sync, so the console's Development schema shows only what some dev device has actually written — an optional attribute no row has filled is absent, and a Production deploy taken from it leaves App Store users' first export carrying that field rejected (sync stalls). The flag calls `initializeCloudKitSchema` once per process, on the first load that mounts a `NSPersistentCloudKitContainer` (sync ON), writing one representative record per entity with every attribute set into every mirrored container's Development schema — both `iCloud.ai.gigaduck.agentrelay` and `.blobs` when the build's profile carries the blobs entitlement; without it the Blobs store mounts local-only and only the first container is filled (the launch log says `Blobs container entitlement missing`). The work holds a content-sync context lease, so flipping sync off mid-run waits or refuses with an active-operations error rather than tearing the stores down under it. Needs a dev-signed run on a real device or the Mac with iCloud signed in (the Simulator never mirrors) — a dev-signed build without an explicit `com.apple.developer.icloud-container-environment` entitlement already runs against Development (the `com.apple.coredata` log names the container `:Sandbox`), so any Xcode-installed device build qualifies, while TestFlight and App Store builds run against Production, where the call refuses (check with `codesign -d --entitlements :- <app>`); watch the console for `CloudKit Development schema initialized`, then refresh the CloudKit Console and deploy each container. Lives in `Services/ConversationStore.swift` (Watch compiles that file, not `QA/`). Debug-only; inert without the arg; pre-added unticked to the shared scheme. |
...
- **CloudKit sync** — cross-device conversation sync needs signed devices.
  On the Mac, inbound changes can lag by minutes even with the push
  entitlement in place: Apple's push service holds the Mac's silent sync
  pushes until the Mac next sends any traffic (the app's own share of the
  delay is about a second). Read it from the Mac, never from app code:
  `/usr/bin/log show --last 20m --style compact --predicate 'process == "apsd" AND (eventMessage CONTAINS "LastFromStorage" OR eventMessage CONTAINS "ExplicitKA" OR eventMessage CONTAINS "receivedPushWithTopic")'`
  — a `LastFromStorage` push right after an `ExplicitKA` keep-alive is a
  held push; the `receivedPushWithTopic … timestamp` field is when Apple
  originated it, so arrival minus that timestamp is the hold time.
