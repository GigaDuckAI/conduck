...
- The named Canvas/Tiles/List control matches what is drawn. All materials and projects support Canvas. Search temporarily shows List; clearing search restores the previous layout, zoom and position. With accessibility text enabled, the control shows List and cannot select a layout the board would ignore.
...
1. Open Work on iPhone, iPad and Mac. The Canvas view should show movable cards and project stacks, with a project picker on a phone and a collapsible rail in a wide window. Switch among Canvas, Tiles and List; relaunch and check the choice remains.
