...
- **A transcription that failed** keeps its recording so the user can retry rather than repeat themselves. Each waiting capture is queued under its own identifier, its record written before its bytes and its removal tombstoned first, so an interrupted arm or discard finishes rather than half-existing; that record outranks the index when they disagree, and one that cannot yet be read defers rather than guessing. A surface reserves the capture it is finishing — by name where it made the recording, the moment it queues it — renews while it works, and confirms it still holds before handing the words on, so two surfaces never finish the same one; an unrenewed reservation lapses, and a held capture is exempt from the clock. Every act is scoped to its holder: finishing, recording again and discarding each release or delete only that surface's own capture, and a lane holds a claim rather than a second copy of the audio. A capture the queue could not keep is still transcribed and delivered — nothing was saved for anyone else to hold, so the copy in hand is the only one. `PendingRetryMetadata.isExpired` reclaims a retryable transcription, never a recording that is a card's only copy — so the card carries a confirmed discard as the only way out. Discard reserves before it asks, so the recording removed is the one on screen; where every waiting recording is held elsewhere it says so and asks nothing, and cancelling hands the capture straight back. Its confirmation says what is actually lost: where the desk already holds the recording, only the queue's second copy. The headless Shortcut route saves the recording *proactively* — when the OS kills that process mid-transcription no error path is left to run — and holds it only as long as the notice it posts takes to invite a retry, so it comes back exactly when the person is told to come for it.
