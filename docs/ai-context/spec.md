...
No backend, no analytics, no telemetry, no crash reporting, no rate-limiting service, no health endpoint, no account system. **The app never makes a network request to anything we operate** — beyond Apple's own services, which are enumerated under "Data, secrets, and what leaves the device", every request goes to the AI the user configured, their speech provider, or their file server. The only publisher-operated destinations are website links the user has to tap, and a Send Feedback item that composes a mail carrying the app version, OS version and device model.

It also never registers for remote notifications and holds no push token; every notification it posts is local. The push entitlement and remote-notification background mode exist only because iCloud sync uses a silent push under the hood. macOS reads that entitlement under its own key. The Mac can still lag by minutes: Apple's push service may hold its sync notifications until the Mac next checks in.
...
Always-on execution — hosting, serving files to an agent, background jobs and long-running work — belongs on the gateway. Conduck keeps its conversation and Work library locally. The device can sleep, suspend or be killed by the OS, so it cannot host an always-on service.

**Why:** the split follows from what each side can actually guarantee. Putting always-on execution on a phone means discovering, repeatedly and in production, that the phone was not running.
...
**No probe may decide from a status alone.** Supported servers can return a nominal success with a control page or sign-in wall instead of the requested representation. Every verdict therefore reads and validates the body.

### A file the agent produces comes back in a folder the app named for that one reply

The gateway connection carries text and nothing else, and the agent platforms that define their own attachment syntax strip it before a reply leaves them. So the app names a folder on a file server both sides can already reach, tells the agent where it is in the outgoing turn, and lists exactly that folder once the reply is in. Nothing outside it is looked at, and reply text never decides what this route lists, hands back, or asks the server for. Every dispatch names its own folder, a retry included, because a folder reused across attempts lets a late write from an abandoned attempt arrive as this reply's work.
...
**A failing lane is backed off, never switched off.** Disabling it would remove the only control that can restore it. The gate is process-local and withholds only an optional file-server probe, never the AI turn. A device with no route and a request the user stopped are evidence about the device, not the server, so they do not narrow capability or lengthen the wait. A passing test, a configuration change, a relaunch, or a user-requested rescan clears the guess.

**A server proved unable to list is not failing.** Its stored verdict bypasses the backoff because re-proving the same permanent limitation on every turn would add a request that cannot change the answer. Conversely, a run in which every newly named folder is reported as already present is treated only as a process-local limit; it changes no stored capability and makes no user-visible accusation.
...
**A turn with no folder is only a fault if the lane was supposed to give it one.** Three folder-less turns are indistinguishable in the record and none is a failure: a turn dictated on the Watch whose lane the phone has not couriered as ready; a turn on a lane that cannot list at all; and a row mirrored from iCloud before the folder attribute existed on it. So the app decides from three live facts instead of from the record — the lane must be failing its check *now*, the turn must have landed after that run of failures began, and the manual filename search must have a name on that reply it could go and ask about. Without the first the notice outlives the outage it describes; without the second one rotated hostname lights up every wrist turn in the thread's history; without the third a greeting answered with a greeting carries a complaint about files neither side mentioned, under a remedy that would answer the tap with nothing. What the third fact costs is stated rather than hidden: the notice reaches exactly as far as the search behind it does, so a reply naming a kind of file that search does not recognise draws nothing, and the surface that answers for the server rather than for any one turn — the lane's own connection test — is where that user is served. The verdict is derived when the thread is drawn and never stored, so it clears the moment the server answers.
...
**The row may never promise a delivery nothing proved.** A persisted remainder keeps the reason that produced it; user copy must not promise future automatic delivery where the limit is permanent.
...
Automatic delivery follows the content-sync choice. The Watch holds no file-server credential and cannot find the file itself, so a capable device creates the row and the shared database mirrors it to the wrist in its own time. That is far too slow for someone still looking at the reply they just dictated, so the phone also hands the description across the paired link the moment it attaches the row: the name, the size, the type, and which turn it belongs to. Never the bytes, never a preview, never the server's address, and never a credential — the wrist gains something it can draw, not something it could fetch. It goes out on both channels at once, because they answer different questions: the queued channel survives sleep and reachability gaps, and the interactive one fires only when the Watch app is awake, which is exactly when a second matters. Duplicates are harmless because the wrist recognises a description by its turn and the file's identity. On the wrist it is an overlay and never a local database write, which would make it a record in its own right that mirrors back and stands beside the phone's forever.

**Reading filenames out of reply text happens only when the user asks for it.** It is available on every agent turn, including ones that never named a folder, and what it finds is labelled as found on the file server rather than as produced by the reply, because a name lifted from a reply is a name the reply chose and the folder had no part in. It never spends bytes without a tap. The names in a reply are read without one in a single place — deciding whether the folder-less notice has a search worth offering on that turn — and that reading is local, costs no request, and reaches no server.
...
The two readings get different sentences and different fates. Provable absence says the key is missing and points at where to add one; that verdict is final, and a capture refused on it is discarded, because repeating it cannot succeed. An unanswered read says only that the key could not be read, invites the user to unlock and try again, and is retryable — so the capture is kept.

**Why the distinction is worth the second read:** the alternative tells a user with a perfectly good key that they never set one up, on a device where the remedy shown does nothing, and throws away what they just said in the process.

**A capture refused after the user has spoken is kept wherever the surface has somewhere to keep it.** In the app and on the wrist that is a preserved recording or a queued entry with a visible retry. On CarPlay it depends on the lane. A chat capture has no such place and none is invented: that refusal loses it, and earns its keep by being true and leaving the driver able to say it again. A Work capture is preserved through the pending-retry lane the headless Shortcut uses — its bytes are queued before the original is deleted, so every refusal below that point costs the words and keeps the recording, and the car says so. Where preservation is claimed it must be real — the promise that a recording is saved is made only by the surfaces that can see the bytes landed, never by the code that merely attempted to write them.

**The wrist queue decides what is permanent by asking whether the error is retryable**, not by matching particular failures, because an entry deleted on a recoverable error takes the user's words with it. A chat entry is still bounded by age and by count, and the expiry notice claims no cause, since a queued capture can reach the iPhone every time and still be refused there.

**Every sentence names the device that actually failed.** The same words render on the wrist, on the paired phone's lock screen and in the car, so a bare "this device" is never enough where two devices are involved.
...
**The Watch is a full participant.** Its reads and failure acknowledgements follow the conversation-sync preference. Because that write decides for devices the user is not holding, the wrist gates it harder than any other surface — an active scene is not enough, since the ambient dim keeps a scene active with the wrist down.
...
All functional code, including purchases and access policy, is public. Official builds add private art, identity, signing, the CarPlay entitlement and the subscription product identifier. A community build compiles the CarPlay code and the purchase code alike, but with no entitlement the car scene never activates and with no product identifier nothing is offered for sale; source users may modify limits.
...
Forgetting a gateway destroys its address, token, certificate pin and file-transfer setup. What survives is the monogram, a palette colour, the last name the gateway carried, and the fact that the removal was deliberate, so the conversations it created keep the tag that told them apart. The name is shown in one place only: the Usage history, which still counts that gateway's attempts and can therefore say it was removed rather than merely unavailable. Everywhere else a forgotten gateway reads under the same generic label as one whose roster entry is missing for any other reason, because the badge is what tells conversations apart and a name standing where the interface expects a live gateway would be a placeholder. This makes custom gateways behave like the built-in ones, which keep their badge for free because their letters and colours are compiled in.
...
A peer's disappearance is adopted only from a roster the synced store actually holds. An absent roster key proves nothing — it also describes signed-out or not-yet-synced devices — so reconciliation may preserve local badge context, never records it as a deliberate removal, and never erases secret families or republishes a stale roster.

Custom voice endpoints follow the same proof rule. Only a confirmed deletion moves an active speech pointer back to Apple; a merely stale roster never retargets audio or credentials.
...
The free plan caps how many self-hosted and custom gateways a device keeps configured, bounded by `Constants.maxConfiguredGateways`, and how many Work projects are active at once, bounded by `Constants.maxActiveWorkProjects`. A gateway counts from the moment it is saved, whether or not its credential is readable here, because an unreadable key may merely be still syncing. Hosted models are exempt. Conduck Pro lifts both limits, and only a transaction Apple has verified grants it, signed grace periods included; no preference, local or synced, can.

Going above an allowance never locks anything that exists. The user chooses what stays active: projects not chosen are archived with their materials intact, and gateways not chosen keep their configuration and credentials, so choosing them later costs nothing. Everything stays readable and exportable, a dispatched reply still lands, and a device that comes back online over the allowance loses nothing. Access is a signed deadline re-read on every check, so an offline device keeps Pro until it passes, and a failed status read leaves a still-valid grace deadline standing rather than revoking it. When the store cannot be reached, no price is invented and no purchase button is drawn.

### Work captures stay inert until an explicit project handoff

A capture reaches the Work desk and stops there, filed into a project or loose on Home beside the projects. Nothing on the desk sends anything to a gateway by itself. The only route from the desk to an AI is a project brief, and it goes out only after the user has reviewed the context, task and materials it carries and confirmed the destination gateway. **Do not propose** implicit dispatch of any kind, such as a capture that sends itself because its project already has a conversation: it hands a private thought to an AI on a route the user never watched.

A material may sit in more than one project: moving it replaces only the location it left, adding it leaves the others in place, and every appearance shares one payload and one set of edits, while a brief already sent keeps its snapshot. Deleting a project keeps its conversations and asks whether the materials reviewed into it are kept or deleted everywhere; a kept material loses that one location and returns to Home if nothing else holds it. A file the agent hands back joins the project with its provenance recorded, a gateway's reference to a file is never mistaken for a local copy, and a result enters the next brief only by explicit inclusion, so an unavailable file must be retrieved or excluded first. Editing a material never rewrites context already sent.

Two doors are shaped by the rule. An audio file becomes a playable card only through the desk's attachment button or a drop into Work, and every other door refuses it with the reason, so the desk never becomes a third place audio is kept. The wrist reaches Work only through the chooser Ask opens, never an implicit trigger, and a Work entry in its relay queue is exempt from the age and count that bound the queue — refused at capacity, never evicted — because until the phone confirms the desk has it, the wrist holds the only copy.

### The desk's bytes are proved rather than trusted, and the Watch never holds them

The desk mirrors into the person's own private iCloud when content sync is enabled, bytes included. A payload within `Constants.workboardSyncCeilingBytes` goes through the mirrored store; anything larger stays in a device-local vault and is reattached elsewhere rather than synced. Neither store is trusted on the strength of the write alone: the vault reads its bytes back at the length written, and the mirrored store checks the size and content hash its row recorded, so a refused reattach restores the prior payload when that is lossless. A card names the bytes it was published with, so a peer's upload serves it only once publication completes; a publication that dies before its card leaves a payload row nothing may sweep, because an orphan and an arrival still in flight are indistinguishable, and it goes only with the card's bytes or with the card. A capture whose identifier already names a card of another kind is republished under one escape identifier derived from it, so every process and replay repairs the same card. The payload store mirrors through a CloudKit container of its own, because one container cannot mirror two stores, and the Watch's entitlements name only the first.

Every lane passes a picture through one normalisation point, which keeps a metadata-stripped copy no larger than `Constants.workboardImageMaxPixel` rather than the picture as captured. The rule keys on what the card is, not on what the bytes contain: a picture that already conforms is kept as it is, one the system does not recognise as an image lands as a file card with its bytes untouched, and animations, undecodable pictures and cards published before this rule keep their bytes, replays included.
...
**macOS has no background sessions.** A turn stranded by quitting mid-send has no delegate left to mark it failed — which is why quitting with a turn in flight asks for confirmation there and on no other surface. Both platforms sweep for stranded turns at launch and again after a delay — the second pass exists because a quick relaunch finds the turn not yet old enough to sweep — but the Mac has no live-task registry to exclude in-flight work, which iOS does. **The menu bar owns a Work door of its own**: a global hotkey that captures to the desk, or, in text mode, opens the compose surface Work-only — no Ask affordance, its text held apart from the chat draft, because one surface must never mean one composition when Chat's Return reaches a gateway.

**The Watch is not a remote control for the phone.** It records locally, and where the clip goes depends on the provider: it reaches the phone only when transcription is on-device or a custom endpoint, a cloud speech provider uploads straight from the wrist, and the gateway hop always goes wrist-to-gateway. **A Work voice capture always relays**, whichever provider is chosen, because the phone writes its card. The wrist has text entry but no configuration UI — setup is taught on the phone.

**A CarPlay voice session ends on silence, and the two silence windows are long on purpose.** A session's first listen waits `Constants.carPlayInitialSilenceTimeout` for speech; every listen re-armed after a spoken reply waits `Constants.carPlayFollowUpSilenceTimeout`, the longer of the two. The follow-up window is the expensive one: a driver mid-conversation may need to think or attend to the road, and killing a live conversation is worse than holding the route that long. **A muted session is never signed off for not talking.** Mute cancels the guard and parks the session, holding the route for as long as the driver leaves it parked.

**CarPlay may show a conversation's title and date, and none of its text.** Apple's rules for the voice-based-conversation entitlement forbid message content on a car screen: browsing to pick up a thread is allowed, showing its contents is not. It is also why the picker marks a thread's gateway by colour, not words. **The Work row obeys it by recording only**: one note, no follow-up listen, no Mute, no desk content on the screen — and whether a memo reaching no other person belongs under that entitlement is raised in the App Review notes rather than waited on. CarPlay never activates in a community build — the code ships, but the entitlement is granted per developer team and cannot.

**The share extensions decode nothing** — they copy bytes and exit, and nothing that decodes a payload may be added to them. The system gives an extension a small fraction of the app's memory, and decoding a single modern photo exceeds it; the copy also has to happen inside the system's own callback, because the file it hands over stops existing when that callback returns. The one thing an extension computes is the small document for a shared web page, assembled from text the browser extracted inside the page itself and treated as untrusted input — re-validated and re-clamped before anything is written. Everything else happens in the main app when it drains the shared inbox, which is why a shared item's reply arrives later rather than at once.

**The share extension activates for almost everything**, rather than filtering to types the app understands. That is a product boundary: Conduck forwards to the user's own gateway and cannot know what it will do with a payload. In a Chat capture, anything unrecognised is routed to that gateway's file server; if the bound gateway has none — the hosted lane never does — the whole item fails visibly, with a notification.

---

## Data, secrets, and what leaves the device

**Conversations** live in local databases optionally mirrored into private iCloud; Apple encrypts the mirror and we cannot read it. App Group storage lets headless Shortcuts access the same files from another process. The paired Watch receives file descriptions while content sync is enabled; explicit captures can still cross the phone relay when it is disabled.

**Content sync defaults on and the choice syncs.** Off retains local data and existing cloud copies. Re-enabling reconciles edits and deletions. Settings, keys, and explicit transfers remain independent; Usage clearing stays account-wide. Completion requires both mirrors and cooperating processes to stop. An unreadable sync preference may suspend mirroring, but must not prevent access to existing local data. Offline devices, older builds, and fresh installs awaiting settings cannot be remotely prevented from syncing. Disabling is not cloud deletion or control over device backups.

**What the user has already read** follows conversation sync. The account cutover alone uses the key-value store.

**Secrets.** On iPhone, iPad and Mac every API key and gateway token lives in the keychain marked synchronizable, so it moves between the user's own devices end-to-end encrypted. **The Watch is the exception:** its copies are non-synchronizable and arrive over the phone-to-watch relay rather than through iCloud Keychain, because the wrist cannot be the place a user types a key.

Over the network a secret is sent only to the service it authenticates, and never logged or shown in an error message. Two movements are not network sends and easy to forget: the phone hands the active speech key and gateway token to the paired Watch over the device-to-device relay, and the pairing exporter renders a gateway token back into a setup code for another of the user's devices.

A platform trap: a synchronizable keychain item is a *different* item from a non-synchronizable one with the same name, so an operation that omits the flag silently finds nothing rather than failing.

**Identity** is a locally generated identifier in the keychain. There are no accounts.

**Audio** never enters a conversation, never syncs with one, and is never a Work voice note: that note is its words. An audio file attached in Work is a desk material whose bytes ride the desk's lane.

It is *not* memory-only: transcription and background upload need a file on disk, so a recording goes to scratch storage and is deleted when the operation ends, success or failure, with a launch sweeper for what a crash stranded. Two paths deliberately hold one longer, inside the app's container:

- **A transcription that failed** keeps its recording so the user can retry rather than repeat themselves. Each waiting capture is queued under its own identifier, its record written before its bytes and its removal tombstoned first, so an interrupted arm or discard finishes rather than half-existing; that record outranks the index, and one that cannot yet be read defers rather than guessing. A surface reserves the capture it is finishing the moment it queues it — by name where it made the recording — renews while it works, and confirms it still holds before handing the words on, so two never finish the same one; an unrenewed reservation lapses, a held capture is exempt from the clock, and finishing or discarding releases or deletes only that surface's capture; recording again hands the previous one back, never deleting the only copy. A chat capture the queue could not keep is still transcribed and delivered: nothing saved is nothing to contend over. `PendingRetryMetadata.isExpired` reclaims a retryable transcription, never a Work recording whose words are unwritten, so a confirmed discard is the only way out — it reserves before asking and returns the capture on cancel — and names what is lost: the recording, for good. Every lane saves the recording *before* the speech hop — a process killed mid-transcription leaves no error path to run.
- **A relayed Watch recording** is written to the Watch's container *before* the first delivery attempt, so process death cannot strand a clip already spoken. Bounded by `AppleRelayPendingQueue` and deleted the moment the phone claims it — a Work clip only once the phone confirms it has the recording — with an orphan sweep both ways at startup.

Two smaller rules about audio on disk, easily undone:

- **Scratch files must carry a filename prefix the sweeper recognises.** Without one a file is not merely unswept but unreclaimable — no rule broad enough to catch it could spare other frameworks' files in the same shared directory. That mistake has been made three times, so a test scans for it.
- **Capture filenames are random, not timestamped, and the sweeper logs nothing.** The names contain nothing sensitive, but a listing of timestamps would disclose when the user was recording.

**Outbound traffic** goes to Apple — the private iCloud mirror, the key-value store, and the on-device speech-model download — and otherwise to exactly three destinations, all chosen and paid for by the user: their speech provider, the AI they configured, and a gateway's file server where it has one. Attachments sent to a file server keep their metadata; the copy sent inline is downsized with metadata stripped.

One non-obvious threat: the audio package brings in a model-hub client and a machine-learning runtime transitively, so code capable of fetching a model from a third-party host **is compiled into the shipped binary**. Nothing calls it: the voice-activity model is loaded from the app bundle, and a missing resource fails the CarPlay session closed rather than fetching one. The guarantee is a property of the call graph, not the dependency list, which is why "make it degrade gracefully" is the wrong instinct: it would mean a silent third-party request from someone's car.

---

## Usage measurement, and what the ledger never holds

Conduck keeps a private ledger of gateway attempts — one record for each time a turn is dispatched to the user's AI — alongside the conversations, in the same local database and optional private iCloud mirror. It exists so the app can tell the user how their own setup is behaving. The measurement is as local as the conversation it describes, and there is nowhere for it to go.
...
**There is no Sync Now control**: the platform offers no way to force a fetch or export. Account warnings cover actionable outages: signed out, restricted or out of space. Intentional OFF is not an outage; an unfinished sync-preference change is surfaced separately until the local transition is complete.

**Choosing a different gateway inside a CarPlay session does not change the app's default.** It sets an override that lives and dies with that connection, and names a gateway, never the desk: a Work mode reset by every reconnect hands a private thought to an AI on one tap. Calling the settings screen's setter would silently re-point the phone, iPad and Mac.
