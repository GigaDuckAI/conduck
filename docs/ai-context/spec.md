...
Work is one desk per person, made by the first capture and never deleted. Every capture surface lands on it, and no code path leads from it to a gateway.

The desk lives in the person's own private iCloud, bytes included: a payload within `Constants.workboardSyncCeilingBytes` syncs, anything larger stays in the device-local vault behind a reattach, and neither is durable until its bytes read back at the length written, so a refused reattach returns the previous payload. A card names the bytes it was published with, so a peer's upload serves it only after a completed publication; a publication dying before its card strands a payload row nothing may sweep, since an orphan and an arrival in flight are indistinguishable, and it goes only with the card's bytes or the card. One process imports a capture and one publishes a payload, behind App-Group locks; a claim the app cannot hand back is retaken by the recovery pass, and a recapture repairs a bytes-less card but never one newer than the capture it replays. A capture whose identifier already names a card of another kind is republished under one escape identifier derived from it, so every process and replay repairs the same card; a second refusal is terminal, and the capture is retired by a verified copy before the queue lets the original go — one already on disk verified rather than trusted, an interrupted one kept beside it, and nothing retired ever claimed, requeued or swept. A voice note is durable and playable before the speech hop wherever it was spoken; a retry republishes one whose first write failed and degrades to a note only if its card is gone. The payload store mirrors through a CloudKit container of its own, because one container cannot mirror two stores, and the Watch's entitlements name only the first. Deploy model 16 to production CloudKit before release.

A card carrying a picture shows it: the preview is minted at the one desk write and again at a reattach, on the synced lane only, and a row carrying none is filled by a bounded repair pass that stamps no `updatedAt`, since an unasked-for preview must not read as an edit. An image opens one desk-wide gallery of every openable picture, vault included; a file or a recording opens in Quick Look; every card shares its original bytes. All ask the card-action policy first, so a card still waiting for its bytes is neither page, preview nor share.

A Shortcut's files ride the same envelope queue the share sheet writes into, under identifiers taken over the staged copy, not the source read, so a rerun repairs its own cards, and an audio file becomes a playable card. The wrist reaches Work through the chooser Ask opens on every press — gateways, then Work last, whatever the roster holds — and never by an implicit trigger; it relays the recording to the phone, which publishes it before it transcribes, and keeps its clip until the phone confirms the desk holds it, words or not: a Work entry is exempt from the age and the count that bound that queue, and a capture at capacity is refused rather than an older one evicted. A phone too old to confirm returns words alone, a note under an identifier derived from the request. The Mac menu bar's hotkey captures a screenshot first, then records to the desk or aims its compose surface at Work; a voice capture submits at its stop, publishing picture and recording independently, so a cancel before the stop leaves nothing and one after it keeps what is already durable. CarPlay's picker row records a note, never browses one, stands whatever the roster holds, and writes its words only while it still holds the capture.
...
**The share extension activates for almost everything**, rather than filtering to types the app understands. That is a product boundary: Conduck forwards to the user's own gateway and cannot know what it will do with a payload. Anything unrecognised is routed to that gateway's file server; if the bound gateway has none — the hosted lane never does — the whole item fails visibly, with a notification.

---

## Data, secrets, and what leaves the device

**Conversations** live in a local database mirrored into the user's own private iCloud. Apple encrypts it; we cannot read it; there is no copy anywhere else. One piece travels outside that mirror: the description of a file the phone has just found for a reply, handed to the paired Watch so the row does not wait on sync. It carries no bytes and no credential and is held only until the mirrored row arrives. The relay carries more: a wrist capture's recording crosses it whenever the phone transcribes, and always when the capture is bound for Work, where the phone writes the card first and the transcript or failure code returns afterwards. The database sits in the shared app-group container rather than the app's sandbox, because the headless Shortcut runs in a *different process* against the same file. Sync is off on the Simulator: the mirroring container refuses to start without a signed-in account and a registered container.

**What the user has already read** travels with the conversations rather than staying on the device that read them. It carries when a thread was last looked at and which failed delivery attempt was shown. One account-scoped value in the key-value store is the only part outside that mirror.
...
**Audio** never enters a conversation and never syncs with one; a Work voice note is a desk material whose bytes ride the desk's lane.
...
- **A transcription that failed** keeps its recording so the user can retry rather than repeat themselves. Each waiting capture is queued under its own identifier, its record written before its bytes and its removal tombstoned first, so an interrupted arm or discard finishes rather than half-existing; that record outranks the index, and one that cannot yet be read defers rather than guessing. A surface reserves the capture it is finishing the moment it queues it — by name where it made the recording — renews while it works, and confirms it still holds before handing the words on, so two never finish the same one; an unrenewed reservation lapses, a held capture is exempt from the clock, and finishing, recording again and discarding each release or delete only that surface's capture, which holds a claim, never a second copy. A capture the queue could not keep is still transcribed and delivered: nothing saved is nothing to contend over. `PendingRetryMetadata.isExpired` reclaims a retryable transcription, never a recording that is a card's only copy, so a confirmed discard is the only way out — it reserves before asking, removing the one on screen, asking nothing where every waiting one is held elsewhere, returning the capture on cancel — and names what is lost: where the desk holds the recording, only the queue's second copy. The headless Shortcut and CarPlay routes save the recording *proactively* — a process killed mid-transcription leaves no error path to run — and hold it only as long as the notice inviting a retry.
- **A Watch capture** is written to the Watch's container *before* the first delivery attempt, so process death cannot strand a clip already spoken. Bounded by `AppleRelayPendingQueue` and deleted the moment the phone claims it — a Work clip only once the phone confirms the desk holds it — with an orphan sweep both ways at startup.
...
Conduck keeps a private ledger of gateway attempts — one record for each time a turn is dispatched to the user's AI — alongside the conversations, in the same local database and the same private iCloud mirror. It exists so the app can tell the user how their own setup is behaving. The measurement is as local as the conversation it describes, and there is nowhere for it to go.
