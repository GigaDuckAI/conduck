...
The desk lives in the person's own private iCloud, bytes included: a payload within `Constants.workboardSyncCeilingBytes` syncs, anything larger stays in the device-local vault behind a reattach, and neither is durable until its bytes read back at the length written, so a refused reattach returns the previous payload. A card names the bytes it was published with, so a peer's upload serves it only after publication completes; a publication dying before its card strands a payload row nothing may sweep, since an orphan and an arrival in flight are indistinguishable, and it goes only with the card's bytes or the card. One process imports a capture and one publishes a payload, behind App-Group locks; a claim the app cannot hand back is retaken by recovery, and a recapture repairs a bytes-less card but never one newer than the capture it replays. A capture whose identifier already names a card of another kind is republished under one escape identifier derived from it, so every process and replay repairs the same card; a second refusal is terminal, and the capture is retired by a verified copy before the queue lets the original go — one already on disk verified rather than trusted, an interrupted one kept beside it, nothing retired ever claimed, requeued or swept. A voice note is durable and playable before the speech hop, wherever spoken; a retry republishes one whose first write failed, degrading to a note only if its card is gone. The payload store mirrors through a CloudKit container of its own, because one container cannot mirror two stores, and the Watch's entitlements name only the first. Deploy model 17 to production CloudKit before release.
...
A Shortcut's files ride the same envelope queue the share sheet writes into, under identifiers taken over the staged copy, not the source read, so a rerun repairs its own cards, and an audio file becomes a playable card. The wrist reaches Work through the chooser Ask opens on every press — gateways, then Work last, whatever the roster holds — never by an implicit trigger; it relays the recording to the phone, which publishes before transcribing, and keeps its clip until the phone confirms the desk holds it, words or not: a Work entry is exempt from the age and the count that bound that queue, and a capture at capacity is refused, never an older one evicted. A phone too old to confirm returns words alone, a note under a request-derived identifier. The Mac menu bar's hotkey captures a screenshot first, then records to the desk or aims its compose surface at Work; a voice capture submits at its stop, publishing picture and recording independently; the recording names its picture, and the desk draws that pair as one card, an orphan alone. A cancel before the stop leaves nothing and one after it keeps what is already durable. CarPlay's pill opens that chooser, one gateway included, and Work stands on the root without any; the row records, never browses, writing only while holding the capture.
