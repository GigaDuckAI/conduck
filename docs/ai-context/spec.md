...
Above an allowance, users explicitly select active items, archiving other projects while retaining inactive gateway configuration and credentials. Content stays readable/exportable; dispatched replies finish. Offline convergence is nondestructive. Purchases preserve editors without saving credentials or dispatching work.

## Work captures stay inert until an explicit project handoff

Captures never dispatch themselves; project captures use that project, others remain unfiled. Home holds project containers and explicitly placed materials. Materials may occupy several locations: moving replaces only the source; adding preserves others. Search returns each material once. Filing shares payload and edits across appearances; sending retains a snapshot. Deleting a project preserves conversations and explicitly keeps or deletes reviewed materials everywhere. Keeping removes only that project’s location; unplaced materials return to Home. Opening projects, transient presentation and viewport changes preserve desk arrangement and membership. Organization syncs with content; zoom stays local. Handoffs include reviewed context, task and materials after destination confirmation. Returned files join projects with provenance; recovery preserves result deletion and later filing. Gateway references remain distinct from local copies. Results need explicit inclusion; unavailable files require retrieval or exclusion. Edits never rewrite sent context. Conversation retry and routing rules apply.
