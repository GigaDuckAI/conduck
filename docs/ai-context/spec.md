...
A card carrying a picture shows it: the preview is minted at the one desk write and again at a reattach, on the synced lane only, and a row carrying none is filled by a bounded repair pass that stamps no `updatedAt`, since an unasked-for preview must not read as an edit. An image opens one desk-wide gallery of every openable picture, vault included, decoded within a bound; a file or a recording opens in Quick Look; every card shares — the original bytes, copied before the sheet opens — from its menu or the gallery page in view. All ask the card-action policy first, so a card still waiting for its bytes is neither page, preview nor share.

A Shortcut's files ride the same envelope queue the share sheet writes into, under identifiers taken over the staged copy, not the source read, so a rerun repairs its own cards, and an audio file becomes a playable card. The wrist relays a Work recording to the phone, which publishes it before it transcribes, and keeps its clip until the phone confirms the desk holds it, words or not: a Work entry is exempt from the age and the count that bound that queue, and a capture at capacity is refused rather than an older one evicted. A phone too old to confirm returns words alone, a note under an identifier derived from the request. The Mac menu bar's hotkey records straight to the desk or aims its compose surface at Work; CarPlay's picker row records a note, never browses one, and writes its words only while it still holds the capture.
