...
All functional code, including purchases and access policy, is public. Official builds add private art, identity, signing and CarPlay. Community builds omit CarPlay and official subscriptions; source users may modify limits.
...
A peer's disappearance is adopted only from a roster the synced store actually holds. An absent roster key proves nothing — it also describes signed-out or not-yet-synced devices — so reconciliation may preserve local badge context but never erase secret families or republish a stale roster.
...
`Constants.maxConfiguredGateways` counts saved self-hosted/custom connections, including unreadable credentials; hosted models are exempt. `Constants.maxActiveWorkProjects` limits active projects. Apple-verified Pro lifts both, including signed grace periods; preferences never grant access.

Above an allowance, explicit selection archives other projects and retains inactive gateway configuration/credentials. Content remains readable/exportable; dispatched replies finish. Offline convergence preserves data. Purchases retain editors without saving credentials or dispatching work.

## Work captures stay inert until an explicit project handoff

Captures never dispatch: project captures stay there; others remain unfiled. Home holds projects and placed materials. Materials occupy multiple locations; moving replaces only the source, adding preserves others. Search returns materials once. Filing shares payload and edits; sends retain snapshots. Project deletion preserves conversations and explicitly keeps or deletes reviewed materials everywhere. Keeping removes that location; unplaced materials return Home. Projects replace main content; returning restores Home’s arrangement. Home previews allow filing without changing navigation or capture destination; preview lists own project drops and block underlying input. Navigation and zoom never change membership. Organization and colors sync with content; zoom stays local. Handoffs include reviewed context, task and materials after destination confirmation. Returned files join projects with provenance; recovery preserves deletion and later filing. Gateway references differ from local copies. Results require explicit inclusion; unavailable files require retrieval or exclusion. Edits never rewrite sent context. Conversation retry and routing rules apply.

The desk mirrors into the person's own private iCloud when content sync is enabled, bytes included: a payload within `Constants.workboardSyncCeilingBytes` uses the mirrored store, anything larger stays in the device-local vault behind a reattach, and neither is durable until its bytes read back at the length written, so a refused reattach restores its prior payload when lossless. A card names the bytes it was published with, so a peer's upload serves it only after publication completes; a publication dying before its card strands a payload row nothing may sweep, since an orphan and an arrival in flight are indistinguishable, and it goes only with the card's bytes or the card. One process imports a capture and one publishes a payload, behind App-Group locks; a claim the app cannot hand back is retaken by recovery, and a recapture repairs a bytes-less card but never one newer than the capture it replays. A capture whose identifier already names a card of another kind is republished under one escape identifier derived from it, so every process and replay repairs the same card; a second refusal is terminal, and the capture is retired by a verified copy before the queue lets the original go — one already on disk verified rather than trusted, an interrupted one kept beside it, nothing retired ever claimed, requeued or swept. A Work voice note reaches the desk as words alone, wherever spoken; its recording is kept only until those words are written, and a failed hop keeps it for a retry. The payload store mirrors through a CloudKit container of its own, because one container cannot mirror two stores, and the Watch's entitlements name only the first.

Work keeps a metadata-stripped copy at `Constants.workboardImageMaxPixel` from every lane, never the picture as captured; animations, undecodable pictures and cards published before this rule keep their bytes, replays included. Opening and sharing ask the card-action policy first: a card waiting for its bytes is neither page, preview nor share.

A Shortcut's files ride the same envelope queue the share sheet writes into, under identifiers taken over the staged copy, not the source read, so a rerun repairs its own cards. An audio file becomes a playable card only through the desk's own attachment button or a drop into Work; every other door refuses it. The wrist reaches Work through the chooser Ask opens on every press — gateways, then Work last, whatever the roster holds — never by an implicit trigger; it relays the recording to the phone, which queues it before transcribing and confirms it has, and keeps its clip until then: a Work entry is exempt from the age and the count that bound that queue, and a capture at capacity is refused, never an older one evicted. A phone too old to confirm returns words alone, a note under a request-derived identifier. The Mac menu bar's hotkey captures a screenshot first, then records to the desk or aims its compose surface at Work; a voice capture submits at its stop, publishing picture and words independently; the words name their picture, and the desk draws that pair as one card, an orphan alone. A cancel before the stop leaves nothing and one after it keeps what is already durable. CarPlay's pill opens that chooser, one gateway included, and Work stands on the root otherwise; it records, never browses, writing only while holding the capture. The share sheet highlights a new conversation on the default gateway, or the first that can send, and names it on the Send button; Work stands beside it, never among the destinations. A highlight decides nothing, and with no roster nothing is highlighted, since the unnamed route can continue a capture the sheet never showed.
