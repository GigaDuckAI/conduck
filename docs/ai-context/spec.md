...
Captures land on one private desk without dispatch; in-project captures arrive referenced by it, all others unfiled. Projects group materials and gateway-bound conversations. Ungrouping preserves both. Organization syncs privately; viewport zoom stays local. Each conversation receives reviewed project context, its own task and selected materials after explicit confirmation of its destination. Returned files join the project with their origin; gateway references remain distinct from local copies. Results are excluded from new requests until selected; unavailable files require retrieval or exclusion. Project edits never change an existing conversation’s context. Capture never invokes dispatch; ordinary conversation retry and routing rules apply.
