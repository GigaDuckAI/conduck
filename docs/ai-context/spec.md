...
A card carrying a picture shows it: the preview is minted at the one desk write and again at a reattach, on the synced lane only, because vault bytes never sync, and a row carrying none is filled by a bounded repair pass that stamps no `updatedAt`, since a revision moved by a preview nobody asked for reads as an edit. An image opens one desk-wide gallery of every openable picture, whatever lane each lives in, decoded within a bound and released around the current page; a file or a recording opens in Quick Look, where the system's own share also lives. Both ask the card-action policy first, so a card still waiting for its bytes is neither a page nor a preview.

A Shortcut's files ride the same envelope queue the share sheet writes into, under identifiers derived from the capture so a rerun repairs its own cards rather than laying a second set, and an audio file arriving that way becomes a playable card. The wrist relays a Work recording to the phone, which publishes it before it transcribes, and keeps its clip until the phone confirms the desk holds it: a Work entry is exempt from the age and the count that bound that queue, and a capture at capacity is refused rather than an older one evicted. A phone too old to confirm returns words alone, landing as a note under an identifier derived from the request. The Mac menu bar's hotkey records straight to the desk or aims its compose surface at Work; CarPlay's picker row records a note and never browses one.
