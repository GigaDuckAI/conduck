...
**A failing lane is backed off, never switched off**, because switching it off would take away the only control the user has to switch it back on. The absence check sits on the critical path of every send, a pure-text turn included, so a server that has gone away would otherwise add its whole deadline to every message typed. Patience depends on the shape of the failure rather than its severity: a lane that returned no HTTP response at all is paused on the first observation, because the commonest cause is a tunnelled address whose hostname rotated and a host that is not there will not be there next turn either, while a lane that answered unhelpfully must fail repeatedly, because a rejected credential or a server error is transient often enough that one sample is not a diagnosis. An attempt that never left the device charges nothing at all: a phone with no network path, or a dispatch the user's own stop cancelled, is evidence about the device rather than the lane, so the turn goes out folder-less without opening or extending the pause — which is what lets the retry sent once the connection is back witness normally and get its folder. The backoff withholds a request, never the truth. It is process-local health, never persisted and never shown in settings, cleared by a relaunch, by any edit to the address, credential or pin, by a passing test, and by the user asking for a turn to be looked at again. A passing test clears it whatever the listing check concluded: that check settles a claim about the server and may only be made on proof, while the pause is a guess about whether another request is worth spending.
...
A forget performed elsewhere cannot rely on the live change notification alone, because that notification only reaches a running process — the system applies a key-value delta silently while the app is quit and replays nothing at the next launch. So every device also reconciles the synced roster from the key-value store's local cache at launch and on foreground activation. The reconcile only ever *adopts*: it acts on a roster the store actually holds, never treats an absent key as a delete — an absent key is also what a signed-out device reads — and never publishes the local roster upward, which would resurrect gateways a peer had forgotten. It is deliberately not gated on the iCloud-account check that guards the push-up paths, because that check reads the ubiquity identity token, which tracks iCloud Drive, and the key-value store keeps syncing when that token is nil. On a stale roster guess the only action taken is retiring the vanished badge, which self-corrects; erasing key families or stored pointers stays reserved for the explicit forget and the live change path.

The roster of custom voice endpoints reconciles by exactly the same rule, and for the same reason. It carries one extra obligation: a deleted endpoint that was the *active* speech or speech-synthesis provider leaves a pointer naming a server that no longer exists, and unlike a gateway pointer that one still resolves — the transcription path reads the endpoint's address without consulting the roster, so recorded audio and a bearer token would go to an endpoint the user deleted. A confirmed delete therefore falls both pointers back to the built-in Apple provider. Confirmed, not guessed: the reconcile at launch or activation still only adopts the roster and leaves pointers alone, because a roster older than an endpoint this device just created is indistinguishable from a delete.
...
The desk lives in the person's own private iCloud, bytes included: a payload within `Constants.workboardSyncCeilingBytes` rides CloudKit, anything larger stays in the device-local vault behind a reattach, and neither is durable or released until its bytes read back at the length written, so a refused reattach returns the previous payload. A card names the bytes it was published with, so a peer's upload serves it only after a completed publication of them — and a publication dying before its card strands a payload row nothing may sweep, since the two are indistinguishable; the row goes when the card's bytes are replaced or the card is deleted. One process imports a capture and one publishes a payload, app and headless intent alike, behind App-Group locks; a claim the app cannot hand back is retaken by the recovery pass, and a recapture repairs a bytes-less card, though never one newer than the capture it replays — another device's file still arriving. A capture whose identifier already names a card of another kind is republished under one escape identifier derived from it, so every process and replay repairs the same card; a second refusal is terminal: its directory is copied aside and verified byte for byte before the queue lets the original go. A voice note, in the app or from a Shortcut, is durable and playable before the speech hop; a retry republishes one whose first write failed and degrades to a note only if its card is gone. Deploy model 16 to production CloudKit before release.
...
**A CarPlay voice session ends on silence, and the two silence windows are long on purpose.** A session's first listen waits `Constants.carPlayInitialSilenceTimeout` for speech; every listen re-armed after a spoken reply waits `Constants.carPlayFollowUpSilenceTimeout`, the longer of the two. The follow-up window is the expensive one, and it is the deliberate side of the trade: a driver mid-conversation may need to think or attend to the road, and killing a live conversation is a worse failure than holding the audio route — and with it the car's paused radio — for that long after the last reply. The End button and Mute are what make that ceiling affordable. **A muted session is never signed off for not talking.** Mute cancels the guard and parks the session, which holds the route for as long as the driver leaves it parked.

**Both windows end by saying something, and what is said depends on whether the microphone worked.** At expiry the session classifies its own capture pipeline from counters kept during the current listen — what the input tap delivered, what the format converter produced, what the detector inferred — and speaks the ordinary sign-off for a healthy pipeline, including on the cold-connect window, because a modal that simply vanishes reads as a crash. A broken pipeline (no buffers at all, converted samples that are uniformly digital zero, nothing convertible, a detector that ran on nothing or returned non-numbers, or a detector task that threw) gets one spoken line about the microphone instead; which layer failed is a question for the logs, not for the driver. Three things constrain that verdict. Counters are scoped to the current tap epoch and reset whenever an engine reconfiguration reinstalls the tap, or one healthy conversion from before it would mask a pipeline that has since died. An epoch too young to have seen anything convicts nobody: a tap reinstalled a second before the deadline, or a hands-free link still spinning its codec up and delivering digital zeros, has empty counters because it is new rather than because it is broken. And an all-zero *but finite* probability stream is a quiet cabin rather than a fault — telling a silent driver their microphone is broken is the worse error, which is why a dead hands-free path emitting comfort noise is knowingly signed off as quiet. An exit stays silent wherever speaking would escalate the failure instead of explaining it: a session whose audio never came up, one whose capture file or capture engine could not be built, one whose voice-activity model fails to load, and one that has been backgrounded — the first three have no working audio session to speak on, and the last has nobody in front of it.

**Endpointing is quantized, so the tuning dial has dead zones.** The detector consumes fixed 256 ms frames and the first silent frame contributes nothing to its tally, which makes the trailing silence a driver actually feels `(ceil(minSilence / 0.256) + 1) × 0.256`: the configured 1.5 s buys 1.792 s, and every other value inside the same 256 ms band behaves identically. Generous on purpose — ending a turn early costs twice, truncating the thought *and* re-arming the microphone into road noise. `CarPlayVADQuantization.feltEndOfSpeechDelay(minSilence:)` computes what a candidate value buys; guessing at one does not.

**Speech has to be corroborated before it counts, and that rule belongs to the app rather than the library.** The streaming detector reads only threshold, release offset, padding and minimum silence — its minimum-*speech* setting governs a batch segmenter this code never calls — so a single loud 256 ms chunk of road noise is enough to make it declare speech. Two consecutive chunks at or above the threshold are required instead, judged on raw per-chunk probabilities and never on the library's in-speech state: after a blip, hysteresis plus the silence hold keep that state reporting speech across several quiet frames, so a state-based test would approve precisely what the rule exists to reject. An uncorroborated episode is discarded without ending the turn and without disturbing the silence guard, and the stream keeps flowing so a later qualifying pair still counts. The accepted cost is a genuinely short answer that yields only one qualifying chunk: it is dropped, and the driver repeats themselves with the microphone still live. Two details are load-bearing at the boundary — cancelling the guard takes a generation token as well as invalidating the timer, since a timer that has fired has already queued its work and invalidation cannot recall it; and a deadline reached with one qualifying chunk pending defers by a single frame rather than ending a session on someone mid-word.

**An empty result costs a retry, not the session.** An empty transcript and a speech provider's own no-speech verdict mean the same thing to a driver, so both say so and listen again; the second in a row signs off, which is what bounds a cabin producing nothing but noise. No converse turn token is minted for silence — a token spent there would name a turn that no reply can match. Everything on this path keys off a monotonic listen counter rather than that token, which exists only once a non-empty transcript has been stored: without the counter, a slow speech response landing after the driver has ended one session can re-arm or kill the session that replaced it.
...
**Conversations** live in a local database mirrored into the user's own private iCloud. Apple encrypts it; we cannot read it; there is no copy anywhere else. One piece of conversation data travels outside that mirror: the description of a file the phone has just found for a reply, handed to the paired Watch so the row does not wait on sync. It carries no bytes and no credential, is held only until the mirrored row arrives, and exists because the wrist cannot discover such a file itself. The relay carries more: a wrist capture's recording crosses it whenever the phone transcribes, and the transcript or failure code returns. The database sits in the shared app-group container rather than the app's sandbox, because the headless Shortcut runs in a *different process* against the same file. Sync is off on the Simulator by design: the mirroring container refuses to start without a signed-in account and a registered container.
...
Over the network a secret is sent only to the service it authenticates, and never logged or shown in an error message. Two movements are not network sends and easy to forget: the phone hands the active speech key and gateway token to the paired Watch over the device-to-device relay, and the pairing exporter renders a gateway token back into a setup code the user shows another of their devices.
...
- **A transcription that failed** keeps its recording so the user can retry rather than repeat themselves. Each waiting capture is queued under its own identifier, its record written before its bytes and its removal tombstoned first, so an interrupted arm or discard finishes rather than half-existing; that record outranks the index when they disagree, and one that cannot yet be read defers rather than guessing. A surface reserves the capture it is finishing, by name where it made the recording, renews while it works, and confirms it still holds before handing the words on, so two surfaces never finish the same one; an unrenewed reservation lapses. `PendingRetryMetadata.isExpired` reclaims a retryable transcription, never a recording that is a card's only copy — so the card carries a confirmed discard as the only way out. The headless Shortcut route saves it *proactively*: when the OS kills that process mid-transcription no error path is left to run, and a clip saved in advance is the only way back.
- **A Watch capture** is written to the Watch's container *before* the first delivery attempt, so process death cannot strand a clip already spoken. Bounded by `AppleRelayPendingQueue`, deleted the moment the phone claims it, with an orphan sweep both ways at startup.
