...
Captures never dispatch themselves; in-project captures use that project, others remain unfiled. Materials remain in the complete collection after filing, with one optional project home and multiple gateway-bound conversation uses. Filing copies no payload; sending retains a snapshot. Project deletion preserves conversations and explicitly keeps or deletes reviewed materials; kept materials return together. Organization follows content sync; viewport zoom stays local. Project handoffs include reviewed context, task and chosen materials after destination confirmation. Returned files join projects with provenance; recovery preserves result deletion and later filing. Gateway references remain distinct from local copies. Results require explicit inclusion; unavailable files require retrieval or exclusion. Later edits never rewrite sent context. Conversation retry and routing rules apply.
