...
Captures never dispatch themselves; in-project captures use that project, others remain unfiled. Materials have a single project home and multiple gateway-bound conversation uses. Filing copies no payload; sending retains a snapshot. Project deletion preserves conversations and explicitly keeps or deletes reviewed materials; kept materials return together. Organization syncs privately; viewport zoom stays local. Requests include reviewed project context, task and chosen materials after destination confirmation. Returned files join projects with provenance; gateway references remain distinct from local copies. Results require explicit inclusion; unavailable files require retrieval or exclusion. Later edits never rewrite sent context. Conversation retry and routing rules apply.
...
**What the user has already read** travels with the conversations rather than staying on the device that read them. One account-scoped value in the key-value store is the only part outside that mirror.
