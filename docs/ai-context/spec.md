...
Captures never dispatch themselves; in-project captures use that project, others remain unfiled. Home holds project containers and explicitly placed materials. A material can appear in several locations; moving replaces only the source location, while adding preserves the others. Search finds each material once across those locations. Filing copies no payload; edits reach every appearance, while sending retains a snapshot. Project deletion preserves conversations and explicitly keeps or deletes reviewed materials everywhere; keeping removes only the deleted project’s location, returning otherwise unplaced materials to Home. Opening a project replaces the main content area while preserving Home’s arrangement for return. Home’s explicit contents preview permits filing without changing navigation or capture destination; its list owns project drops and blocks input behind it. Navigation and zoom never alter membership. Organization and project colors follow content sync; zoom stays local. Project handoffs include reviewed context, task and chosen materials after destination confirmation. Returned files join projects with provenance; recovery preserves result deletion and later filing. Gateway references remain distinct from local copies. Results require explicit inclusion; unavailable files require retrieval or exclusion. Later edits never rewrite sent context. Conversation retry and routing rules apply.
