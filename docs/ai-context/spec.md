...
| What is the current value of some limit? | `Conduck/Conduck/Utilities/Constants.swift` |
...
### A file the agent produces comes back by name, and the name is never enough

The gateway connection carries text and nothing else. There is no attachment channel, and the agent platforms that define their own attachment syntax strip it before a reply leaves them. So an agent hands a file back by writing it into the file server both sides can already reach and naming it in ordinary reply text, and the app matches that name against the server and offers the download. The claim and the bytes therefore travel on two channels that know nothing about each other, which is why a delivery needs evidence from both — a name in the reply *and* the file confirmed on the server. Neither one alone produces anything the user can tap.

**A file server must prove it can say no before it is allowed to say yes.** Confirming a name is a probe, so the status-alone rule above already applies — but reading the body is not sufficient by itself here, because the body of a wrong answer can be perfectly well formed. A static host with a catch-all fallback returns that fallback with a valid byte range for a file that was never written: the range describes the representation the server chose, and attests to nothing about where the request was routed. So before any confirmation is believed, the app asks the same server for a key that cannot exist and requires a definite miss. A server that answers everything answers that too, and disqualifies itself.

**A file that is not there yet is not the same as a file that is not coming.** Nothing orders the write against the reply, so a first look that finds nothing is evidence about timing, not about failure. A miss leaves the turn open and only a look taken past a grace horizon closes it, and outcomes that say nothing about the file at all — an unreachable server, a refused certificate, a rejected credential — never close it and are held apart from a real absence, so that a broken file server is never reported as a broken promise. Retrying is bounded by backing a uselessly-answering lane off on a widening interval rather than by switching it off, because switching it off would also take away the only control the user has to switch it back on.

**When a file is named and never arrives, the app says so.** Silence is the wrong default for a product whose failing part is usually the user's own setup: a reply that promises a file and delivers nothing, with no explanation anywhere in the app, leaves the user with nothing to act on. The notice is derived on demand rather than stored — recomputed from the reply text, the turn's existing downloads and the lane the conversation currently points at — so it disappears by itself the moment any of those change, and no stored verdict can outlive the situation it described. It names only files the app actually looked for, so widening the recognition rules in a later build cannot make an old turn accuse the server of losing a file nobody ever asked it for.

**Why:** the file server serves the agent's own workspace, written by the same process that writes the reply — so what is on disk is not an independent second source, and a folder shared across conversations means unprompted files cannot be attributed to a turn at all. Evidence has to start from the reply. And because the reply is attacker-shaped in the threat model, the second source has to be something the reply cannot fake, which is a server demonstrating it distinguishes present from absent.

**Rejected:** treating directory membership as a delivery signal, in any form — snapshot-and-diff, an outbox convention, or auto-offering a file no reply mentioned; a structured manifest or sentinel block in the reply, which is a second permanent wire convention and harder for an agent to satisfy than saying a filename; persisting the missing-file verdict, which creates a stored claim that must then be reconciled against a world that has moved on; and reporting probe results back to the agent, which would turn the user's own file server into an existence oracle for whatever is on the other end of the gateway.
...
**Why it is worth writing down:** the protocol refactor is the obvious tidy-up and it has a real threshold. It was chosen against while there is essentially one non-network provider, because a protocol whose only purpose is to abstract a single case adds indirection and removes nothing. Reopen it at a *seventh* non-network provider — not at a general feeling that the dispatch looks untidy.
...
The pointer to "the conversation a capture continues" is written and read **only by headless captures** — the Shortcut intent and the Watch. The in-app thread, the share drain and CarPlay neither write it nor consult it; they append to the conversation the user is actually in. A surface that started consulting it would silently retarget the thread on the user's screen.

Deep links from a notification bypass the landing preference entirely, and the menu bar, Watch and CarPlay ignore it, because none of those is a cold launch into a main window.
...
Four things about these differences are load-bearing:
...
**Conversations** live in a local database, mirrored into the user's own private iCloud database. Apple encrypts it; we cannot read it; there is no copy anywhere else. The database sits in the shared app-group container rather than the app's own sandbox, because the headless Shortcut runs in a *different process* and has to read and write the same file. Sync is off on the Simulator — the mirroring container refuses to start without a signed-in account and a registered container — so simulator runs are local-only by design, not by accident.
...
CI runs the cheap script checks first, then the full iOS and watchOS suites, and a separate macOS job for the tests that need a real signing identity.
