...
A card carrying a picture shows a preview, minted on the synced lane only and backfilled without stamping `updatedAt`, since an unasked-for preview must not read as an edit. An image card keeps a metadata-stripped copy at `Constants.workboardImageMaxPixel`, never the picture as captured, from every lane; animations, undecodable pictures and cards published before this rule keep their bytes, replays included. An image opens one desk-wide gallery of every openable picture, vault included; a file or a recording opens in Quick Look; every card shares the bytes it holds. All ask the card-action policy first, so a card still waiting for its bytes is neither page, preview nor share.
