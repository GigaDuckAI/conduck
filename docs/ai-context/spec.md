...
**A failing lane is backed off, never switched off**, because switching it off would take away the only control the user has to switch it back on. The absence check sits on the critical path of every send, a pure-text turn included, so a server that has gone away would otherwise add its whole deadline to every message typed. Patience depends on the shape of the failure rather than its severity: a lane that returned no HTTP response at all is paused on the first observation, because the commonest cause is a tunnelled address whose hostname rotated and a host that is not there will not be there next turn either, while a lane that answered unhelpfully must fail repeatedly, because a rejected credential or a server error is transient often enough that one sample is not a diagnosis. A device that had no network path, and a request the app's own stop cancelled, charge nothing at all — on the check before a dispatch and on the listing after a reply alike: each is evidence about the device rather than the lane, so the turn goes out folder-less, or stays open with nothing said about it, without opening or extending the pause. That is what lets the attempt made once the connection is back ask normally and get its answer. The exemption is drawn on those two and no others: a refusal decided before any connection is still a statement about the address the lane carries. The backoff withholds a request, never the truth. It is process-local health, never persisted and never shown in settings, cleared by a relaunch, by any edit to the address, credential or pin, by a passing test, and by the user asking for a turn to be looked at again. A passing test clears it whatever the listing check concluded: that check settles a claim about the server and may only be made on proof, while the pause is a guess about whether another request is worth spending.
