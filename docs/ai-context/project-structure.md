...
| `Conduck/Configs/` | `Identity.xcconfig` — every identity-bearing value as a build variable rather than a hardcoded string: bundle identifiers, App Group and iCloud container, display name, version numbers, the entitlements variant, the App Store identifier, the subscription product identifier and the sign-in callback scheme. The Keychain group is not set here; the entitlements derive it from the bundle-identifier base. Ships with community defaults; a private, gitignored override file alongside it substitutes another identity without touching source. |

---

## The main app — `Conduck/Conduck/`

One target covers iPhone, iPad and Mac. The Mac build is a full Dock application *and* a menu-bar agent; both live here. The target's platform settings, and the iOS share extension's device families, also list visionOS, which nothing else in the repository acts on: no source file branches on it, no scheme or workflow names a visionOS destination, and no continuous-integration job builds one. Read the supported set as the three platforms above until that setting is either removed or backed by a build.

| Folder | What lives there |
|---|---|
| *(top level)* | The app entry point and the top-level window and scene wiring shared by all three platforms. |
| `Models/` | The data types the whole app agrees on — conversation and message records, the error type, the user-preference enumerations, request and reply shapes. Most are structure without behaviour, and many are compiled into the Watch app as well; the exceptions are the usage aggregation that derives the dashboard's figures from the ledger, and the display identity that resolves a usage record's gateway to a name at draw time. One thing here is not a type at all: `Models/Conversations.xcdatamodeld`, the versioned Core Data model behind the conversation store; `.xccurrentversion` names the model the app opens. Work organization uses additive, relationship-free metadata entities beside the existing capture records. Each version of it is additive-only, and every earlier version is still a live upgrade path on a device that has not opened the app in a long time, which makes it the highest-risk edit in the repository. |
...
| `Services/` | App services: recording, conversation and usage persistence, settings, capture and retry queues, content-sync lifecycle and monitoring, purchase verification and the Pro access cache, image and text extraction, permission checks, device identity, and phone-side Watch transport. Work-specific capture processing lives in `Services/Workboard/`. |
...
| `ViewModels/` | The state between views and services: conversation list and detail, Work capture and project organization, the usage dashboard, settings, diagnostics, and pairing import. |
...
| `Views/Settings/` | The largest folder in the app. Every settings screen for every platform, plus the guided gateway-setup flow. iPhone/iPad and Mac have deliberately separate screen hierarchies here rather than one adaptive layout. The Pro paywall and the usage dashboard live here too, beside the screens that gate on them. |
...
| `QA/` | The QA harness — debug flags, an accessibility hierarchy dump, an on-screen banner, screenshot seeding. Every file here compiles out of Release builds behind its own `#if DEBUG`; the folder has no special target membership. |
...
The same split also runs through the app's entitlements, which are four files rather than one — `Conduck/Conduck/Conduck-Community.entitlements` and `Conduck/Conduck/Conduck-Official.entitlements` for iOS, selected by a build variable, and a `-macOS` twin of each that the macOS product signs instead (a per-SDK build setting), because macOS names the push entitlement under a different key and drops the iOS one. Xcode's capability editor writes only to whichever variant the active configuration selects and never touches the others, so a capability added through the capabilities UI lands in one build and is silently missing from the rest. Edit all four files by hand, and check the variants you are not building.
...
| Conversation or Work storage, or content sync | `Conduck/Conduck/Services/` and `Conduck/Conduck/Services/Storage/` |
| Usage measurement — what is recorded, or how it is shown | `Conduck/Conduck/Services/` for the writes; `Conduck/Conduck/Models/` for the aggregation; `Conduck/Conduck/ViewModels/` and `Conduck/Conduck/Views/Settings/` for the dashboard |
| Pro access, purchases, or a free-plan allowance | `Conduck/Conduck/Services/` for the store and the access cache, `Conduck/Conduck/Views/Settings/` for the paywall, and `Conduck/Conduck/Utilities/Constants.swift` for the allowances themselves |
...
| A user-visible string | `Conduck/Conduck/Localizable.xcstrings`, and `Conduck/ConduckWatch Watch App/Localizable.xcstrings` if the wrist shows it too; each share extension carries a catalogue of its own as well. They are independent catalogues, so a string more than one target displays is written in each — including a string that lives in a file the Watch compiles from the app target, because the lookup still resolves against the Watch's own bundle |
