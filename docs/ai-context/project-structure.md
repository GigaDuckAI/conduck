...
| `Services/` | Everything that does work rather than draws: audio recording, the conversation store, settings, the share-sheet capture queues and the drainer that turns a queued Chat capture into a sent turn, permissions, iCloud sync monitoring, retry bookkeeping. The Work queue's own drainer is one folder down. |
