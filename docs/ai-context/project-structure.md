...
| `Models/` | The data types the whole app agrees on — conversation and message records, the error type, the user-preference enumerations, request and reply shapes. Those are structure without behaviour, and many of them are compiled into the Watch app as well. One thing here is not a type at all: `Models/Conversations.xcdatamodeld`, the versioned Core Data model behind the conversation store; `.xccurrentversion` names the one the app opens, `Conversations 17`, whose only addition over its predecessor is the optional identifier a Work recording carries to name the screenshot taken with it. Each version of it is additive-only, and every earlier version is still a live upgrade path on a device that has not opened the app in a long time, which makes it the highest-risk edit in the repository. |
...
| `Services/` | Everything that does work rather than draws: audio recording, the conversation store, settings, the share-sheet capture queues and the drainer that turns a queued Chat capture into a sent turn, permissions, iCloud sync monitoring, the phone half of the wrist relay — which routes each request by the destination the wrist stamped, publishing a Work recording to the desk before it transcribes and saying so in the reply — and the queue of unfinished voice captures — keyed by capture, so a second arming never displaces the first, and each entry holds its own recording until it lands or the person discards it. A surface claims one entry at a time — by name when it made the recording itself, newest-first when a person taps Retry — under a lease it renews while it works and confirms it still holds before handing the words on, so two of them cannot finish the same recording and a transcription that runs long cannot have its recording taken underneath it. An entry's record is written before its bytes and its removal is tombstoned before anything goes, so an arm or a discard interrupted by process death resolves on the next launch instead of half-existing; that record is the authority, and an index row disagreeing with it is rewritten from it rather than trusted. The Work queue's own drainer is one folder down. |
...
| `Services/Workboard/` | Everything the Work desk needs that is not a view: the one place that decides where a payload's bytes live, the device-local vault holding the ones that stay, the filesystem lock that stops the app and the headless intent process publishing one card's payload at the same instant, the drainer that empties the App-Group capture queue onto the desk and retires a capture it can never turn into cards by copying the whole claimed directory aside and verifying the copy before the queue lets go of the original, the two-phase voice capture that publishes a recording before transcription is attempted so a failed transcription costs the words and never the audio, the derivation of the one escape identifier a capture's bytes land under when the capture's own identifier already names a card of another kind, and the live adapter between the desk's presentation model and the store, which is also where a recording naming a picture is folded into that picture's card — first eligible candidate of the identifier it names and the escape derived from it, lowest identifier winning where several name one picture, and nothing ever dropped, so a recording whose picture is absent simply stands on its own. Bytes within `Constants.workboardSyncCeilingBytes` ride the person's own private CloudKit and reach their other devices; anything larger stays in the vault, and another device shows the card and asks for the file rather than a payload it cannot open. What a surface outside the app is handed is always a disposable copy, never the vault's own file — one export snapshot serves both Quick Look and Share, names the copy by a recognised extension in the title, then the stored MIME type, then the vault leaf, and gives every copy its own directory so the age sweep retires each on its own. That mirror is the only place anything here goes: no code in this folder reaches an AI, and there is no server of ours for it to reach. |
| `ViewModels/` | The state that sits between views and services — conversation list and detail, the Work desk's presentation boundary, settings (split across several files by area), the diagnostics runner, the pairing import flow. |
...
| `Views/Workboard/` | The Work desk and the Work/Chats shell that hosts it, which keeps both drafts alive across a switch between the two. Work is one desk, so there is one board: cards laid out by a mosaic placement engine, a composer pinned under them — which is the only route to a typed note — the link sheet reached from that composer's attach menu, the voice-capture sheet and the playable voice-note card it leaves behind, the sync notice raised for the three account states a person can act on, where a Work deep link lands, and the one-time tutorial. A card carrying a picture is the picture, and opening one raises a desk-wide gallery of every openable image on the board; a file or a recording opens in Quick Look instead. A picture that folds a recording draws one card with two controls — the tile opens the gallery, a band across its bottom plays the recording and shows its words — and offers each file its own open, share and reattach; the board renders the cards a person can see, so the order it sends the store expands each folded card back to its two materials. Every card shares from its menu and the gallery from the page in view: a coordinator owned by the shell re-checks the card, prepares the original bytes behind a visible state, then presents the system's share UI itself, so there is no second tap. The consumable route a Shortcut leaves behind, so that a cold launch still reaches the recorder, lives here too. These views collect material and hold no transport of their own; where a card's bytes then live is decided in `Services/Workboard/`. |
...
| `CarPlay/` | The CarPlay scene. It has its own recorder, its own audio session handling and its own end-of-speech detection rather than reusing the phone's, because the car is a hands-free multi-turn surface with different interruption rules. The picker also carries a one-shot row that records a note onto the Work desk, offered whatever the gateway roster holds and listed first where no gateway is, and the gateway chooser repeats that row last as an action that stores no destination: that lane preserves its compressed bytes for retry before it transcribes, offers End and no Mute, and never draws a card on the car screen. |
...
| Path | What it is |
|---|---|
| `Conduck/ConduckShareExtension/` | The iOS share-sheet extension. One destination list — every configured gateway, the recent chats, then Add to Work as the last row — and nothing is picked until the person picks it; a gateway pick writes the send manifest, the Work row writes an inert envelope for the desk. Both land in shared App-Group inboxes the app drains when active. |
...
| The Work desk, or a card on it | `Conduck/Conduck/Views/Workboard/` and `Conduck/Conduck/ViewModels/WorkboardViewModel.swift` |
| Landing something on the Work desk from a new surface | `Conduck/Conduck/Services/ConversationStore+Workboard.swift` — one write there owns the desk's identity and the repair a replayed capture needs, and every capture surface in the app target calls it rather than making a card its own way. The Watch is the exception the shared-source list forces: that file is not in its target, so `Conduck/ConduckWatch Watch App/WorkboardCaptureIntent.swift` restates the same contract for the one shape the wrist produces. The identifier a refused capture escapes to is derived in `Conduck/Conduck/Services/Workboard/WorkMaterialCollisionEscape.swift`, and every lane that republishes derives it there rather than minting one, which is what makes two processes and any later replay repair the same card. If the capture carries bytes, `Conduck/Conduck/Services/Workboard/` is what decides where they live. Out-of-process and multi-file lanes reach that write indirectly: the share extensions and the Shortcuts file action publish a capture envelope into the App-Group inbox and let `Conduck/Conduck/Services/Workboard/WorkCaptureDrainer.swift` land it, while every recorded lane — the menu bar, CarPlay, and the wrist capture the phone publishes — goes through `Conduck/Conduck/Services/Workboard/WorkVoiceCaptureCoordinator.swift`, whose recovery runs through the pending-retry queue rather than the inbox |
