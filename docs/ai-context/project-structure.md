...
| `MenuBar/` | The Mac menu-bar agent, its popover, and the user-configurable global hotkeys — one of which captures straight to the Work desk, behind a capture panel that outranks the popover's other content and a Work-only state of the compose surface. That state keeps its own composition: one surface may host both aims, but a composition aimed at the desk is never offered to a Return that reaches a gateway. |
