...
//
// The one publication transaction behind every inert Work capture, shared by
// the main app and both share extensions. The iOS and macOS extensions carry
// verbatim mirrors because each extension is a separate process and target;
// keep the declarations byte-identical across the three copies from `import
// Foundation` onward. `WorkCaptureSharePublisherTests` guards the mirror and
// owns the behavioural coverage of every failure path.
