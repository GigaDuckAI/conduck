...
struct WorkDeskProjectSurface: View {
        ...
        HStack(spacing: 4) {
            Menu {
                Button(LocalizedStringResource("workdesk.conversation.new", defaultValue: "New conversation…"),
                    ...
                    .disabled(!workspace.currentProjectAllowsNewActivity)
            ...
            if !workspace.isSelecting && workspace.currentProjectAllowsNewActivity {
    ...
    private func traySelectionActions(_ project: WorkDeskProjectRecord) -> some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ...
                }.buttonStyle(.bordered).disabled(workspace.selectedIDs.isEmpty || !workspace.currentProjectAllowsNewActivity)
                Menu {
                    ...
                    ForEach(workspace.organization.availableProjectDestinations.filter { $0.id != project.id }) { target in
    ...
    private func newConversation(_ project: WorkDeskProjectRecord) {
        ...
        guard workspace.currentProject?.id == project.id else { return }
        workspace.beginConversation(resolver: resolver)
    }
