...
//
// Share Extension — the on-disk CONTRACT between the (capture-and-queue)
// Share Extension appex and the main-app `SharedInboxDrainer`. The
// appex copies each shared `NSItemProvider` representation into an App-Group
// inbox envelope dir + writes ONE `manifest.json` (this type) describing the
// envelope, then exits; the drainer reads it on the next foregrounding and runs
// the real pipeline (classify → upload → append → dispatch).
//
// DEPENDENCY-FREE Foundation (no app types) on purpose: the appex carries a
// VERBATIM MIRROR of this file (`ConduckShareExtension/SharedInboxManifest.swift`)
// so the 120 MB-capped extension stays self-contained — separate compilation
// modules, one source file each, no cross-target membership. Keep it that way:
// any reference to `RemoteAgentRef`, `AttachmentDraft`, etc. would drag the whole
// app graph into the extension AND break the mirror. KEEP THE TWO FILES
// BYTE-IDENTICAL below their headers — the pinned `encoded()` bytes (`.iso8601` +
// `.sortedKeys`) are the cross-process contract; a change on one side that isn't
// mirrored makes the drainer unable to decode what the appex wrote.
//
// FORWARD-COMPAT: decode is TOLERANT — every field added after v1 must default
// on a decode miss so an envelope written by a NEWER appex (e.g. user upgraded
// the appex but a relaunched OLDER drainer reads it, or vice-versa across a
// staged rollout) still decodes instead of poisoning the whole inbox. The
// schema version `v` is recorded for diagnostics + future branch points; it is
// NOT a hard gate (a reader never rejects on `v` alone).
