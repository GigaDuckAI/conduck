// Conduck
// WebPageCapture.swift  (ConduckShareExtensionMac appex — CANONICAL copy)
//
// Safari page-text capture: the pure contract between the appexes'
// `NSExtensionJavaScriptPreprocessingFile` probe (`ConduckWebCapture.js`) and
// the main-app `SharedInboxDrainer`. Owns payload parsing, the synthetic
// Markdown document + its filename, both truncation stages (capture-side
// 128 KiB re-clamp + drainer-side no-file-server inline clamp), and the
// URL-equivalence gate for the envelope's `urls[]`.
//
// COMPILED INTO THREE MODULES: this macOS appex (synchronized group), the iOS
// appex (VERBATIM MIRROR at `ConduckShareExtension/WebPageCapture.swift`), and
// the MAIN APP (explicit pbxproj file reference on THIS copy — the
// `ShareTargetFilter` precedent) so the drainer + `ConduckTests` reach it.
// KEEP THE TWO FILES BYTE-IDENTICAL BELOW THEIR HEADERS — drift guard:
// `ConduckTests/WebPageCaptureTests`.
//
// NO SwiftUI / AppKit / UIKit imports — pure Foundation. The moment this file
// touches an app or UI type it stops compiling in the other two modules.
