...
//
// The pick type `ShareTarget` plus the pure, view-free search, threshold,
// destination-list, opening-pick and Send-label rules for the macOS Share
// Extension's one destination list — held here rather than in `ShareView` so they
...
//
// `ShareTarget` lives here for the same reason: the adapter that turns a snapshot
// into the opening pick, and the one that decides what the Send button names,
// both speak in it, and both are covered by `ShareTargetFilterTests` rather than
// by reading a SwiftUI body. The view keeps only the two localized strings those
// rules cannot reach.
//
