...
//
// Two structural guards sit beside the behaviour, because both properties are
// invisible to any assertion about one card. The projection answers its two
// questions ONCE for a whole board: a per-card vault probe or blob lookup queues
// a board's worth of round trips behind every other write in flight. And it
// answers the blob question from a metadata projection that never names
// `payload`: faulting a board's bytes in to ask how big they are is the exact
// cost the payload store exists to avoid.
...
final class WorkboardAvailabilityTests: XCTestCase {
    ...
    func testAvailabilityIsResolvedOncePerFetchRatherThanOncePerCard() throws {
        let store = try source("Conduck/Services/ConversationStore+Workboard.swift")

        XCTAssertFalse(
            store.contains("workAssetVault.contains("),
            """
            A read path probes the vault key by key again. Each probe suspends on \
            the vault actor, so a board's worth of cards queues that many round \
            trips behind every write in flight. `WorkAssetVault.urls(for:)` \
            resolves the whole set in one hop.
            """
        )
        XCTAssertEqual(
            store.components(separatedBy: "workMaterialRecords(for: ").count - 1, 2,
            """
            Both fetch paths — one material, and the whole board — must project \
            through the one batch helper. A second projection is how the single \
            card and the board start disagreeing about what "available" means.
            """
        )
        XCTAssertEqual(
            store.components(separatedBy: "availableLocalKeys: availableLocalKeys").count - 1, 1,
            "records are built in exactly one place, from sets resolved once"
        )
    }
