...
final class WorkDeskCanvasInteractionTests: XCTestCase {
    ...
    func testWorkspaceRetainsDistinctViewportsAcrossRemountsAndProjects() async {
        ...
        let desk = workspace.canvasSession(for: .desk), projectSession = workspace.canvasSession(for: project)
        desk.transform = .init(scale: 0.8, offset: CGSize(width: -180, height: -300))
        workspace.search = "An idea"
        workspace.suspend()
        XCTAssertTrue(workspace.canvasSession(for: .desk) === desk)
        XCTAssertEqual(workspace.canvasSession(for: .desk).transform.offset.width, -180)
