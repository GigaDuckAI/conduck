...
final class WorkboardDeskSurfaceDriftGuardTests: XCTestCase {
    ...
    func testArrangementControlsRemainMacOnlyAndUseModelLayout() throws {
        ...
        // The column mounts the cards through `boardSurface` — the drag-aware
        // chain wave A split off the body — so the desk is read at that seam.
        let cards = try XCTUnwrap(
            WorkboardSourceDirectives.enclosingConditions(of: "boardSurface", in: column),
            "The board's column no longer mounts `boardSurface` — the cards themselves are gone."
        )
        XCTAssertEqual(
            WorkboardSourceDirectives.ownership(of: cards), .unconditional,
            """
            `boardSurface` is now behind a compilation condition. The cards are the desk on every \
            platform; only the control row above them is Mac-only.
            """
        )
        let surface = try RefusalLaneSource.trailingClosure(
            after: "private var boardDropSurface: some View",
            in: board,
            path: Self.boardPath
        )
        let mountedCards = try XCTUnwrap(
            WorkboardSourceDirectives.enclosingConditions(of: "boardContent", in: surface),
            "`boardDropSurface` no longer mounts `boardContent` — the cards themselves are gone."
        )
        XCTAssertEqual(
            WorkboardSourceDirectives.ownership(of: mountedCards), .unconditional,
            "`boardContent` is now behind a compilation condition; the cards are the desk on every platform."
        )
