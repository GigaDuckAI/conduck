...
//   │                                       │     only; the one action lives below)
...
// VStack floor for the primary action), iOS keeps its native nav bar (`Cancel` +
// the "Where to?" title) and moves the action into a bottom composer via
// `.safeAreaInset(edge: .bottom)`. That gives native software-keyboard avoidance
// while keeping the destination list and the action visually distinct.
//
...
// the question, and the button answers only once a row is tapped, so no share is
// routed for the person in either direction. A Work pick reaches
// `onAddToWorkboard` and only the inert Work capture inbox; a gateway pick
// reaches `onSend` and the send manifest. The two cannot be confused, because a
// pick is a `ShareDestination` while the manifest writer takes only a
// `ShareTarget`, which carries no desk case. Neither action fires on appear, the
...
struct ShareView: View {
    ...
    /// question, and the primary button stays disabled until one is tapped.
    ...
    /// default gateway). It has to be tappable, because nothing is pre-selected.
    ...
    /// PINNED floor: message field and the ONE primary action, which names the
    /// picked destination — "Choose a destination" (disabled) until a row is
    /// tapped, then "Add to Work" or "Send now". ⌘-Return fires the same button,
    /// so it can only commit what a disabled button would refuse.
    ...
    /// helper below is bound to ONE inbox by construction and reads no pick.
    private func commit() {
    ...
    /// tapped after the failed attempt began. It has no target parameter, so it
    /// cannot dispatch.
    ...
    private enum Strings {
        static let title = String(localized: "share.destination.title",
            ...
            comment: "Share Extension navigation title over the destination list (gateways, recent chats, Add to Work)")
