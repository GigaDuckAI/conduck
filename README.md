...
- **Hosted models as an on-ramp** — OpenRouter, for multi-turn chat before you run anything of your own. You operate no server on this lane, and it covers chat, not agent tools or file exchange.
...
**Adapter.** The published contract a server meets in order to answer Conduck: the request and reply shapes, and nothing else. Anything that speaks it works, whatever it is written in. [`conduck-connect`](https://github.com/gigaduckai/conduck-connect) grades a server against it with `--check-adapter`.
